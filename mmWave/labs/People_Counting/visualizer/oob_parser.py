...
class uartParserSDK():
    def __init__(self,type='(Legacy) 2D People Counting'):
        ...
        if (type=='(Legacy): Overhead People Counting'):
            ...
            self.vital_signs = 1
            #self.capon3D = 1
        elif (type == "Small Obstacle Detection"):
            self.small_object = 1
        #data storage
        ...
        self.boundBoxStates = [False] * 32 # Initialize to 32 empty zones
...
#It can parse the SDK3.x Compliant People Counting demo "tracker_dpc"
    ...
    # Parse TLV for Occupancy State Machine 
    def parseOccStateMachTLV(self, dataIn):
        occStateMachStruct = 'I'
        occStateMachLength = struct.calcsize(occStateMachStruct)
        try:
            occStateMachData = struct.unpack(occStateMachStruct, dataIn[:occStateMachLength])
            occStateMachOutput = occStateMachData[0]
            for i in range(32):
                # Since the occupied/not occupied flags are individual bits in a uint32, mask out each flag one at a time
                self.boundBoxStates[i] = (occStateMachOutput & (1 << i)) != 0
        except Exception as e:
            print(e)
            self.fail = 1
...
#parsing for Small Object Detection
    def smallObjFrame(self, dataIn):
        #reset point buffers
        self.pcBufPing = np.zeros((5,self.maxPoints))
        headerStruct = 'Q8I'
        ...
        for i in range(numTLVs):
            ...
            if (tlvType == 1):
                ...
                self.parseOccStateMachTLV(dataIn[:tlvLength])
    ...
    def readAndParseUart(self):
        ...
        elif (self.small_object == 1):
            self.byteData = self.smallObjFrame(self.byteData)
        else:
        ...
        if (self.fail):
            if(self.vital_signs == 1):
                ...
                return self.pcBufPing, self.targetBufPing, self.indexes, self.numDetectedObj, self.numDetectedTarget, self.frameNum, self.fail, self.classifierOutput, self.boundBoxStates
            else:
