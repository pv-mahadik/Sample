...
% Scene Run choice: {'WallMountTestArea' | 'OverheadTestArea'};
sceneRun = 'OverheadTestArea';
...
if(liveRecording == true)
    ...
    chirpConfigurationFileName = '..\chirp_configs\pc_6843_3d_aop_overhead_3m_radial_staticRetention.cfg';
    fhistFilePath = '';
...
if(strcmp(sceneRun,'WallMountTestArea'))
    scene.maxPos = [[-4 4] [0 10] [-1 3]]; %xmin,xmax, ymin,ymax, zmin,zmax
    scene.sensorPos = [0, 0, 1.9]; %x,y, and z of sensor
    scene.numberOfBoundaryBoxes = 1;
    scene.boundaryBox(1,:) = [-3.5 0 0 7 10 3]; %x,y,z (left,near,bottom), width, depth, height
    scene.numberOfStaticBoxes = 1;
    scene.staticBox(1,:) = scene.boundaryBox(1,:) + [0.5 0.5 0 -1 -1 0]; %x,y,z (left,near,bottom), width, depth, height
    scene.numberOfTargetBoxes = 1;
    scene.targetBox(1,:) = scene.staticBox(1,:); %x,y,z (left,near, bottom), width, depth, height
    scene.targetBox(2,:) = [0 0 0 0 0 0]; %x,y,z (left,near, bottom), width, depth, height
    scene.azimuthTilt = 0*pi/180;
    scene.elevationTilt = 15*pi/180;
end

if(strcmp(sceneRun,'OverheadTestArea'))
...
set(figHandle,'Name','Texas Instruments - 3D People Counting Demo Visualization','NumberTitle','off')
pause(0.1);
...
% Tab 1: Statistics, [left, bottom, width, height]
hTabGroup(1) = uitabgroup(figHandle, 'Position', [0.0 0.4 0.14 0.6]);
% Tab 2: Point Cloud
hTabGroup(2) = uitabgroup(figHandle, 'Position', [0.14 0.0 0.43 1]);
% Tab 3: Gating and Association
hTabGroup(3) = uitabgroup(figHandle, 'Position', [0.57 0.0 0.43 1]);
% Tab 4: Chirp Configuration
hTabGroup(4) = uitabgroup(figHandle, 'Position', [0.0 0.1 0.14 0.3]);
% Tab 5: Control
hTabGroup(5) = uitabgroup(figHandle, 'Position', [0.0 0.0 0.14 0.1]);
